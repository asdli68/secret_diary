<?php
    ...
    if ($_POST['submit']=="Sign Up"){
        ...
        if($error) $error = "There were error(s) in your signup details:".$error;
            ...
            if($results) $error = "That email address is already reqistered,Do you want to log in?";
            else{
    ...
    if($_POST['submit']=="Log In"){
        $query="SELECT * FORM users WHERE email='".mysqli_real_escape_string($link,$POST['logineamil'])."'AND password='".md5(md5($_POST['loginemail']),$_POST['loginpassword'])."' LIMIT 1";
        $result = mysqli_query($link,$query);
